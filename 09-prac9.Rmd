...
[Andy](https://open.spotify.com/album/6NtEjhPWfZcvJQuvjGX4bk) Haim. Can all play loads of instruments and just switch during live sets. 
...
As it is, the scatter plot shows that, generally, as the $x$ axis *independent variable* (unauthorised absence) goes up, our $y$ axis *dependent variable* (GCSE point score) goes down. This is not a random cloud of points, but something that indicates there could be a relationship here and so I might be looking to reject my null hypothesis. 
...
So first let's again get rid of all the columns we aren't using in this model using as they will just slow us down. At the moment we only care about average GCSE scores and unauthorised absecne in schools. 
...
However, this is a very ugly output and pretty hard to understand. Now remember when we just went through the magic of [broom]? You might have thought that it didn't do very much! This is where it can really be of use. We want to tidy up our models results so we can actually use them. Firstly let's add a new column with mutate (coef_info) then take our model column and using the `map()` function again tidy the outputs (saving the result into that new coef_info column).
...
Now let's have a look at the histogram of our coefficients to see the distribution....
```{r}
coef %>%
...
Here, we give the function our models (`GCSE_models_tidy`), tell it what column the statistics are in (`coef_info`), specify the level of significance (`alpha`). Now if you recall we set the apparent argument to true earlier on and this is because this is a requirement of `int_pctl()`
...
**Confidence interval** = the range of values that a future estimate will be between

**Confidence level** = the percentage certainty that a future value will be between our confidence interval values
...
Ok, so how do we know that our coefficient is the same for all 626 points within our first bootstrap? Well we can check all of them, but this would just print the coefficient info for all 262....
...
The decision is down to you as the modeler - it might be that a transformation doesn't succeed in normalising the distribution of your data or that the interpretation after the transformation is problematic, however it is important not to violate the assumptions underpinning the regression model or your conclusions may be on shaky ground. 
